...
**Success Criteria** (what must be TRUE):
  ...
  3. `/api/rates/refresh` fetches BCV (via cheerio scraping), dolarapi.com parallel rate, and CoinGecko BTC/ETH and upserts results to the `exchange_rates` table — including fallback to last-known-good rate if BCV scrape fails
  4. Supabase Edge Function cron triggers `/api/rates/refresh` hourly (NOT Vercel cron — Hobby plan is daily only); a daily Vercel cron keep-alive ping reaches Supabase to prevent free-tier project pause
...
**Goal**: Users see live BCV, dolarapi.com parallel, and CoinGecko crypto rates on demand, with source attribution and staleness display, refreshing automatically in the background
**Depends on**: Phase 2
...
**Success Criteria** (what must be TRUE):
  ...
  2. User can see the current dolarapi.com parallel USDT/VES rate with a "Paralelo" label and staleness timestamp alongside the BCV rate
...
- [ ] 03-02: Rate display UI — BCV, parallel, CoinGecko cards with source label, staleness, and delta indicator
...
**Success Criteria** (what must be TRUE):
  ...
  4. User sees current BCV, parallel, and BTC/ETH rates in a compact widget on the home screen — rates match what is shown on the full rates page
**Plans**: TBD
